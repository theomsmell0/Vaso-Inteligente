  ...
  void conectaWifi() {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      Serial.println("\nWiFi conectado!");
      digitalWrite(led, HIGH); // Acende o LED ao conectar no WiFi
  ...
  void loop() {
    if (WiFi.status() != WL_CONNECTED) {
      digitalWrite(led, LOW); // Apaga o LED se o WiFi cair
    ...
    if (httpResponseCode > 0) {
      digitalWrite(led, HIGH); // Acende o LED com a resposta bem-sucedida do servidor
      ...
      digitalWrite(led, LOW); // Apaga o LED se a conexão com o servidor falhar
